...
let enemies=[], nextDifficultyAt=150;
...
/* Start */
startBtn.onclick=()=>{ unlockAudio(); startGame(nameInput.value.trim(),idInput.value.trim()); };
quickStartBtn.onclick=()=>{ const u=JSON.parse(localStorage.getItem("seamex_user")); if(u) startGame(u.name,u.id); };
changeUserBtn.onclick=()=>{ returningSection.classList.add("hidden"); loginSection.classList.remove("hidden"); };
