...
function animate() {
    ...
    if(Math.random() < 0.004) {
        ...
        powerups.push({x:Math.random()*(canvas.width-40), y:-50, type: type});
    }
    ...
    powerups.forEach((p,i) => {
        ...
        // Improved collision (standard AABB)
        if (player.x < p.x + 40 && player.x + 85 > p.x && 
            player.y < p.y + 40 && player.y + 85 > p.y) {
            if(p.type === 'SLOW') {
                timeWarpActive = true; 
...
document.getElementById('continue-btn').addEventListener('click', () => {
    ...
    isPaused = false;
    if(!isMuted && gameState === 'PLAYING') bgMusic.play();
    requestAnimationFrame(animate);
});
